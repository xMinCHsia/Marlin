...
budget, verdicts). The `summary_hash` makes two runs comparable: if a
change to the ensemble alters plans for identical inputs, the hash
changes and the regression is visible.

## Scaling

Marlin is stateless between requests - the only shared state is the
acceptance window (per instance). Scale by sharding requests across
instances; the target and drafts are the actual bottlenecks.
